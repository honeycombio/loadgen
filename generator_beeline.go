...
type BeelineTraceGenerator struct {
	depth     int
	spanCount int
...
var _ Generator = (*BeelineTraceGenerator)(nil)

func NewBeelineTraceGenerator(log Logger, opts Options) *BeelineTraceGenerator {
...
func (s *BeelineTraceGenerator) generate_spans(ctx context.Context, depth int, spancount int, timeRemaining time.Duration) {
	if depth == 0 {
		return
	}
	...
	for i := 0; i < nspans; i++ {
		...
		_, span := beeline.StartSpan(ctx, "child") // make this a name later
...
func (s *BeelineTraceGenerator) generate_root(count int64, depth int, spancount int, timeRemaining time.Duration) {
...
func (s *BeelineTraceGenerator) generator(wg *sync.WaitGroup, counter chan int64) {
	s.mut.Lock()
	depth := s.depth
...
func (s *BeelineTraceGenerator) Generate(opts Options, wg *sync.WaitGroup, spans chan *Span, stop chan struct{}, counter chan int64) {
	defer wg.Done()
...
func (s *BeelineTraceGenerator) TPS() float64 {
	s.mut.RLock()
	defer s.mut.RUnlock()
